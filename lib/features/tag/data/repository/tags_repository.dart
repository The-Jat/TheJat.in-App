...
class TagRepository {
  ...
  Future<List<PostTag>> getAllTags({int page = 1}) async {
    final url =
        'https://${WPConfig.url}/wp-json/wp/v2/tags?page=$page&per_page=20';
    try {
      final response = await dio.get(url);
      if (response.statusCode == 200) {
        final decodedList = response.data as List;
        final allTags = decodedList.map((e) => PostTag.fromMap(e)).toList();
        return allTags;
      } else {
        return [];
      }
