...
class PostRepository extends PostRepoAbstract {
  ...
  final String _contentFields =
      'id,title,content,link,featured_image_url,slug,categories,tags,date_gmt,comment_status,author,thumbnail,comment_count,featured_video,featured_youtube_video_url,extra_images,views';
  ...
  Future<List<ArticleModel>> getAllPost({
    ...
    final String url =
        '$baseUrl?page=$pageNumber&per_page=$perPage&_fields=$_contentFields';
  ...
  Future<List<ArticleModel>> getPostByCategory({
    ...
    String url =
        '$baseUrl?categories=$categoryID&page=$pageNumber&per_page=$perPage&_fields=$_contentFields';
  ...
  Future<List<ArticleModel>> getPostByTag({
    ...
    String url =
        '$baseUrl?tags=$tagID&page=$pageNumber&per_page=$perPage&_fields=$_contentFields';
    List<ArticleModel> articles = [];
    try {
      final response = await dio.get(url);
      if (response.statusCode == 200) {
  ...
  Future<List<ArticleModel>> getPostByAuthor({
    ...
    String url =
        '$baseUrl?page=$pageNumber&author=$authorID&status=publish&per_page=$perPage&_fields=$_contentFields';
    debugPrint('Url: $url');
    List<ArticleModel> articles = [];
    try {
      final response = await dio.get(url);
  ...
  Future<List<ArticleModel>> getThesePosts({
    ...
    String url =
        '$baseUrl?include=$posts&page=$page&orderby=include&_fields=$_contentFields';
    List<ArticleModel> articles = [];
    try {
      final response = await dio.get(url);
      if (response.statusCode == 200) {
  ...
  Future<ArticleModel?> getPost({required int postID}) async {
    String url = '$baseUrl/$postID?_fields=$_contentFields';
  ...
  static Future<ArticleModel?> getPostbyID({required int postID}) async {
    ...
    // This static method needs to redefine fields or access it somehow?
    // Since it's static and _contentFields is instance, I'll copy the string here or skip it.
    // The user asked to update "request with a variable".
    // I will include the variable here too. Since it's static, I will just duplicate the logical string or make a constant if I could, but I'll define it locally for simplicity or just skip static method if unused. But let's be thorough.
    const String contentFieldsStatic =
        'id,title,content,link,featured_image_url,slug,categories,tags,date_gmt,comment_status,author,thumbnail,comment_count,featured_video,featured_youtube_video_url,extra_images,views';

    String url = '$mainUrl/$postID?_fields=$contentFieldsStatic';
  ...
  Future<List<ArticleModel>> searchPost({required String keyword}) async {
    String url = '$baseUrl?search=$keyword&_fields=$_contentFields';
    List<ArticleModel> articles = [];
  ...
  Future<List<ArticleModel>> getPostsBySlug({
    ...
    String url =
        'https://${WPConfig.url}/wp-json/wp/v2/posts?slug=$slug&_fields=$_contentFields';
